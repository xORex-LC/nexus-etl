...
def ensure_supported_cache_dataset(cache_gateway, dataset: str | None) -> CommandResult | None:
    """
    Назначение:
        Проверить, что dataset поддерживается cache gateway.
    """
    if dataset is None:
        return None
    if dataset in cache_gateway.list_datasets():
        return None

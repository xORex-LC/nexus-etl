...
from connector.domain.ports.pending_links_repository import PendingLink, PendingLinksRepository, PendingStatus
...
class SqlitePendingLinksRepository(PendingLinksRepository):
    ...
    def add_pending(
        ...
        cur = self.engine.execute(
            ...
            """,
            (dataset, source_row_id, field, lookup_key, PendingStatus.PENDING.value, expires_at),
        )
    ...
    def list_pending_for_key(self, dataset: str, lookup_key: str) -> list[PendingLink]:
        rows = self.engine.fetchall(
            ...
            SELECT pending_id, dataset, source_row_id, field, lookup_key, status, attempts,
                   created_at, last_attempt_at, expires_at, reason
            FROM pending_links

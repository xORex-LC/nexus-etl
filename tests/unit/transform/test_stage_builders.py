...
def test_employees_build_transform_stages_contract():
    ...
    try:
        gateway = _build_gateway(conn=conn)
...
def _build_gateway(*, conn) -> SqliteCacheGateway:
    engine = SqliteEngine(conn)
    cache_repo = SqliteCacheRepository(engine, list(load_cache_dsl_runtime().cache_specs))

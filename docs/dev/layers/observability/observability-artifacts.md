...
├── report_renderer.py     # JsonReportRenderer.render_with_layout  (конкретный класс, без Protocol)
└── plan_writer.py         # write_plan_file_with_layout
...
> **Порта рендерера нет.** Отчёт пишет **конкретный** класс `JsonReportRenderer` (без `Protocol`),
> инстанцируется инлайн в оркестраторе. `IReportRenderer` в коде **отсутствует** — это был бы
> преждевременный порт при единственной реализации. Его уместно ввести в «last responsible moment» —
> когда появится второй рендерер (например, JSONL-леджер отчётов или text-рендерер под
> `reporting.format`). См. [⚠️ Инварианты](#️-инварианты-системы).

### Основные классы / функции

| Класс/функция | Роль | Ключевые методы |
|---------------|------|-----------------|
| `JsonReportRenderer` | Запись отчёта (конкретный класс) | `render_with_layout()` |
| `write_plan_file_with_layout()` | Запись плана в `var/plans/<c>/` | — (single layout-aware writer) |
...
> **Граница с report-слоем**: event-модель и сборка `ReportEnvelope` — в
> [report-delivery.md](../report/report-delivery.md)/[report-models.md](../report/report-models.md).
> Здесь — только **layout-aware/atomic/retention** аспекты персиста (конкретный `JsonReportRenderer`).
...
```python
JsonReportRenderer().render_with_layout(*, envelope, layout, component, now=None) -> str  # путь отчёта
write_plan_file_with_layout(plan_items, summary, meta, layout, component, run_id, generated_at, *, now=None) -> str
```
...
- **Единственный путь записи** — `*_with_layout`; legacy `render(file_base_name=...)` /
  `write_plan_file(report_dir, run_id, ...)` **удалены** (Stage Z), в коде их нет.
...
4. **Инвариант: рендерер отчёта — конкретный класс, без порта**
   - **Что**: `JsonReportRenderer` — единственная реализация; `IReportRenderer`/`Protocol` в коде нет.
   - **Почему важно**: порт при одной реализации — преждевременная абстракция (YAGNI/KISS). Вводить
     `IReportRenderer` — в «last responsible moment», когда появится второй рендерер (JSONL-леджер
     отчётов или text-рендерер под `reporting.format`).
   - **Где проверяется**: `report_renderer.py` (нет `Protocol`); call-site `orchestrator.py` инстанцирует класс напрямую.
...
- [Report Delivery](../report/report-delivery.md) / [Report Models](../report/report-models.md) — сборка `ReportEnvelope`
...
|------|-----------|-------|
| 2026-06 | Создан документ (DEC-002 Stages 3,5,6) | xorex-LC |
| 2026-06 | Сверка с кодом: убран несуществующий `IReportRenderer` (порта нет, только конкретный `JsonReportRenderer`); убраны legacy `render()`/`write_plan_file()` (удалены Stage Z); добавлен инвариант «рендерер без порта / last responsible moment» | xorex-LC |

...
> **Актуально для runtime registry**: словарный DSL читает секцию `dictionaries` из
> активного registry-файла. По умолчанию это `datasets/registry.yml`, но путь может
> быть переопределён через `dataset.registry_path` или `ANKEY_DATASET__REGISTRY_PATH`.
...
```
[active registry file]  [datasets/dictionaries/*.dictionary.yaml]  [datasets/dictionaries/*.manifest.yaml]
         ↓                               ↓                                        ↓
  [loader.py]  ←─────────────────────────────────────────────────────────────────┘
  load_*_spec()
         ↓
...
|---------|------|--------|------------|
| Registry | active registry → секция `dictionaries` | `DictionaryRegistrySpec` | Control plane: какие словари существуют и где их specs |
| Spec | `datasets/dictionaries/*.dictionary.yaml` | `DictionarySpec` | Lookup-схема, источник данных, политика дубликатов |
| Manifest | `datasets/dictionaries/*.manifest.yaml` | `DictionaryManifestSpec` | Fingerprint-метаданные CSV snapshot (SHA-256, row_count) |

### Registry DSL

**Расположение**: Секция `dictionaries` в активном registry-файле

```yaml
# datasets/employees.registry.yaml
dictionaries:
  version: 1
  manifest: dictionaries/ankey.dictionary.manifest.yaml
  items:
    departments:                            # Ключ registry (должен совпадать с spec.dictionary)
      spec: dictionaries/departments.dictionary.yaml     # Относительный путь от datasets/
      enabled: true                         # Отключить без удаления: enabled: false
    job_title:
      spec: dictionaries/job_title.dictionary.yaml
...
dictionary: departments            # Имя словаря (должно совпадать с ключом registry)

source:
  format: csv                      # В v1 поддерживается только csv
  location: ../dictionaries/departments.csv  # Относительный путь от datasets/
  csv:
    delimiter: ";"                 # Разделитель (по умолчанию: ",")
    has_header: true               # Есть ли строка заголовков (по умолчанию: true)
    encoding: utf-8-sig            # Кодировка файла (по умолчанию: utf-8)
    null_values: ["NULL"]          # File-level null markers; по умолчанию тоже ["NULL"]

schema:                            # Псевдоним: data_schema (alias в Pydantic)
  key_column:
    name: name                     # Колонка для lookup (ключ поиска)
...
**Инварианты `schema`**:
- `key_column.name` — непустая строка lookup-key
- `key_column` не имеет nullable semantics
- `value_columns` — непустой список объектов `{name, nullable}`
- `key_column.name` **не должен** присутствовать среди `value_columns[*].name`

**Пример невалидной конфигурации**:
```yaml
# ❌ Ошибка: key_column.name 'code' присутствует в value_columns[*].name
schema:
  key_column:
    name: code
  value_columns:
    - name: code      # Нарушение инварианта
      nullable: false
    - name: name
      nullable: false
...
**Расположение**: `datasets/dictionaries/*.manifest.yaml`
...
- `source.csv.null_values`, `schema.key_column.name`, `schema.value_columns[*].name`, `schema.value_columns[*].nullable`, `schema.normalized_key.ops` — lookup-схема
...
# Доступ к полям:
print(spec.data_schema.key_column.name)        # "code"
print(spec.data_schema.value_columns[0].name)  # "name"
```
...
**Инварианты**:
- `data_schema.key_column.name` не входит в `{col.name for col in data_schema.value_columns}`
...
| Функция | Назначение |
|---------|-----------|
| `load_dictionary_registry_spec(path?)` | Загрузить registry из файла или из активного registry path |
...
1. Загрузка registry (lines 133)
   ...
   - Canonical path: active registry file → секция `dictionaries`
...
4. **Обновить manifest** (вычислить fingerprints):
   ...
   Добавить в актуальный manifest-файл словарей (например, `datasets/dictionaries/ankey.dictionary.manifest.yaml`):
   ```yaml
   items:
     departments:
...
**Формат registry** (активный registry file):
```yaml
dictionaries:
  version: 1
  items:
...
schema:                     # Обязателен; псевдоним для data_schema в Pydantic
  key_column:
    name: <column>          # Обязателен
  value_columns:
    - name: <column>        # Обязателен, список не пустой
      nullable: false
...
**Обязательные поля spec**: `dictionary`, `source.format`, `source.location`, `schema.key_column.name`, `schema.value_columns`
...
- **Отдельный manifest-файл для всех словарей**: Все fingerprints живут в одном общем manifest-файле (например, `datasets/dictionaries/ankey.dictionary.manifest.yaml`), а не в отдельных файлах рядом со spec. Это упрощает atomic-обновление и аудит.

### 🚨 Failure Modes

| Исключение | Условие возникновения | Поведение системы | Как обработать |
|------------|----------------------|-------------------|---------------|
| `DslLoadError(DICT_DSL_REGISTRY_INVALID)` | Невалидный YAML в активном registry, отсутствующая секция (strict mode) | Fail-fast при старте | Проверить `dataset.registry_path` / `ANKEY_DATASET__REGISTRY_PATH`, убедиться что секция `dictionaries` валидна |
| `DslLoadError(DICT_DSL_SPEC_INVALID)` | Невалидный `*.dictionary.yaml`: пустой `value_columns`, `key_column.name` в `value_columns[*].name`, запрещённая op | Fail-fast при загрузке spec | Исправить YAML-файл словаря согласно сообщению ошибки |
...
| 2026-05-05 | Синхронизирован новый dictionary contract: active registry path, `null_values`, object-shaped `key_column` и nullable `value_columns` | xORex-LC |

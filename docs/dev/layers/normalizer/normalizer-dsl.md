...
Декларативная конвертация source-side литералов в canonical `bool`. В отличие от
`to_bool`, принимает набор допустимых значений из DSL и остаётся sink-agnostic:
результатом всегда будет `True`, `False` или `None`.
...
```
1. Загрузить active registry file (_load_registry_or_raise)
...
**Активный registry file — центральный реестр:**
...
### Настроить compile-policy через active registry file
...
Источник передаёт `"false"` (строка) → `parse_bool` → `False` (bool Python).
Если источник передаёт `"unknown"` — ошибка `DSL_OP_FAILED`.
...
Строгость: `to_bool` принимает только `"true"`/`"false"`. Для source literals вроде
`"0"`, `"1"`, `"yes"` или `"no"` используйте `parse_bool` с декларативными
`true_values/false_values`. `validate_sink_row` с `check_types=True` проверяет уже
получившееся canonical bool-значение, а не исходный literal.
...
**Где происходит target-specific bool serialization (`1/0`, `"yes"/"no"`) ?**

Не в normalizer. Нормализация приводит значение к canonical `True/False/None`.
Target-side представление описывается в `sink.serialize` и применяется на payload boundary.
...
| 2026-05-05 | Добавлено описание `parse_bool`, target-side bool serialization boundary и обновлены примеры под active registry path | xORex-LC |

...
Phase 1 bootstrap допускает cached YAML lookup прямо из `ecs_transform`, чтобы не вводить отдельный
registry object до появления Pydantic loader-а. Это не финальная форма: во Phase 2 `ecs_transform`
должен получать уже валидированный registry инъекцией, а сбой загрузки taxonomy не должен валить
logging hot path.
...
| `trace.id`, `labels.pipeline_run_id`, `service.type`, `event.dataset` | context binding + ECS transform | `bind_observability_context(...)`, alias registry |
...
- Reserved-field guard: call-sites не передают `component` как log kwarg, кроме controlled context binding;
  итоговый ECS field для компонента — `service.type`.
...
| `connector.usecases` не импортирует `structlog`, stdlib `logging`, `connector.infra.logging` | AST ratchet test с временным allowlist | Ноль нарушений к концу ECS |
...
| Zone adapters не импортируются в domain | AST architecture tests; import-linter остаётся для общих layer contracts | Ноль |
...
в structlog как exception object / `exc_info=(type(exc), exc, exc.__traceback__)`, а не сериализовать
его в `fields` и не заменять на ambient `exc_info=True`.
...
#### Семантика run lifecycle actions

- `run-completed` с `event.outcome=failure` — graceful command completion с non-zero diagnostic exit.
- `run-failed` — unhandled command-level exception. Phase 1 пока не мигрирует legacy exception path;
  подключение `run-failed` остаётся задачей Phase 3, чтобы не дублировать события failure completion.
...
`var/logs/enricher/` → `service.type` начнёт врать про размещение и ломать ES-корреляцию.
...
- Enforcement: `ecs_transform` берёт `component` из контекста (reserved-ключ) и маппит его в
  `service.type`; guard-тест должен проверять `service.type ∈ ServiceComponent` для controlled
  lifecycle/correlation events.
...
- `component` → `service.type` (= `ServiceComponent`, из контекста).
- `stage` → `nexus.stage.name` (стадия пайплайна).
- `scope` → `nexus.subsystem` (под-область внутри компонента).

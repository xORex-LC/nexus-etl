...
| Файл | Изменение |
|------|-----------|
| `connector/delivery/cli/containers.py` | Добавить `VaultContainer`; обновить `vault_startup_resource` (добавить guard); удалить `_VaultReadProviderRuntime`, `_VaultRetentionRuntime`; `ensure_vault_startup_ready()` deprecates |
| `connector/delivery/commands/enrich.py` | Временно: использовать `VaultContainer` напрямую (до Шага 5 — AppContainer) |
| `connector/delivery/commands/import_plan.py` | Аналогично `enrich.py` |
| `connector/delivery/commands/import_apply.py` | Аналогично; убрать 3 уровня try/finally для vault |

### Инварианты

1. **Vault engine открывается ≤ 1 раза на invocation**: `SqliteContainer.vault_engine` — `Singleton`
2. **`ensure_vault_startup_ready()` не вызывается** в command handlers: её роль в `vault_ready` Resource
3. **`_VaultReadProviderRuntime` / `_VaultRetentionRuntime` удалены**: нет объектов, открывающих собственный vault engine
4. **Vault Factory не кешируется**: каждый вызов `vault.read_service()` возвращает новый экземпляр
...
|-----------|---------|---------------------|
| `ensure_vault_startup_ready()` | Deprecates | Логика перенесена в `vault_startup_resource` |
| `_VaultReadProviderRuntime` | Удаляется | Vault engine — Singleton из SqliteContainer |
| `_VaultRetentionRuntime` | Удаляется | Vault engine — Singleton из SqliteContainer |
| `build_secret_provider()` | Deprecates | Заменяется `VaultContainer.read_service()` |
| `build_secret_retention_hook()` | Deprecates | Заменяется `VaultContainer.retention_service()` |
| `enrich.py`, `import_plan.py`, `import_apply.py` | Средний рефактор | Vault-объекты получают из `VaultContainer` |
